...
write_pcm_main:
        ...
        if (pw.message != 0) {
            //r.ds = r.cs;
            logger.log(Level.INFO, pw.allload_mes); // "ＰＣＭ定義中"の表示
        }

        r.bx = 30; // pw.pcmwork_ofs; // cs:[pcmwork_ofs]
        r.ax = (short) (pcmData[r.getBx()] + pcmData[r.getBx() + 1] * 0x100); // ds:[bx]	;AX=PCM Next Start Address
...
p86load_error:
    ...
    private void allload_exit1() {
        if (pw.message != 0) {
            r.setDx(0); //    mov dx,offset exit1_mes
        }
        r.ax = 1; // PCMが定義出来ません。
    ...
    private void check_pmdb2() {
        ...
        if (pw.pcm_gs_flag != 0)
            break cpb_stc_ret; // ; ERROR Return
...
o4601x:
        ...
        r.cx--;
        if (r.getCx() != 0) break fast_store_loop;
...
o4600:
    ...
    private void key_check_reset() {
        ...
        r.ax = (short) pw.wait_clock; // _wait_clock[bx]
